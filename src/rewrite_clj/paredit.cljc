(ns rewrite-clj.paredit
  ...
  (:require [rewrite-clj.zip :as z]
            ...
            #?(:clj [rewrite-clj.node.stringz :as sn] 
               :cljs [rewrite-clj.node.stringz :as sn :refer [StringNode]])

            [clojure.string :as cstring])
  #?(:clj (:import (rewrite_clj.node.stringz StringNode))))
...
(defn exception [msg]
  #?(:clj (Exception. "Remove at top")
     :cljs (js/Error. "Remove at top")))
...
(defn- ^{:no-doc true} create-seq-node
  ...
  (case t
    ...
    :map (nd/map-node v)
    :set (nd/set-node v)
    (throw (exception (str "Unsupported wrap type: " t)))))
...
#_(defn kill-at-pos
  ...
  [zloc pos]
  (if-let [candidate (findz/find-last-by-pos zloc pos)]
    (cond
